# 1 "src/isa/riscv64/system/mmu.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 2 "src/isa/riscv64/system/mmu.c" 2
...
# 3 "src/isa/riscv64/system/mmu.c" 2
...
uint8_t *get_pmem();
# 4 "src/isa/riscv64/system/mmu.c" 2
...
# 5 "src/isa/riscv64/system/mmu.c" 2
...
word_t pmp_tor_mask();
# 6 "src/isa/riscv64/system/mmu.c" 2
...
# 5 "src/isa/riscv64/system/../local-include/intr.h" 2
...
# 7 "src/isa/riscv64/system/mmu.c" 2
...
# 33 "src/isa/riscv64/system/mmu.c"
static inline uintptr_t VPNiSHFT(int i) {
  return (12) + 9 * i;
}
static inline uintptr_t VPNi(vaddr_t va, int i) {
  return (va >> VPNiSHFT(i)) & 0x1ff;
}

static inline 
# 40 "src/isa/riscv64/system/mmu.c" 3 4
             _Bool 
# 40 "src/isa/riscv64/system/mmu.c"
                  check_permission(PTE *pte, 
# 40 "src/isa/riscv64/system/mmu.c" 3 4
                                             _Bool 
# 40 "src/isa/riscv64/system/mmu.c"
                                                  ok, vaddr_t vaddr, int type) {
  
# 41 "src/isa/riscv64/system/mmu.c" 3 4
 _Bool 
# 41 "src/isa/riscv64/system/mmu.c"
      ifetch = (type == MEM_TYPE_IFETCH);
  uint32_t mode = (mstatus->mprv && !ifetch ? mstatus->mpp : cpu.mode);
  
# 43 "src/isa/riscv64/system/mmu.c" 3 4
 ((void) sizeof ((
# 43 "src/isa/riscv64/system/mmu.c"
 mode == MODE_U || mode == MODE_S
# 43 "src/isa/riscv64/system/mmu.c" 3 4
 ) ? 1 : 0), __extension__ ({ if (
# 43 "src/isa/riscv64/system/mmu.c"
 mode == MODE_U || mode == MODE_S
...
# 43 "src/isa/riscv64/system/mmu.c"
 "mode == MODE_U || mode == MODE_S"
# 43 "src/isa/riscv64/system/mmu.c" 3 4
 , "src/isa/riscv64/system/mmu.c", 43, __extension__ __PRETTY_FUNCTION__); }))
# 43 "src/isa/riscv64/system/mmu.c"
                                         ;
  ok = ok && pte->v;
  ok = ok && !(mode == MODE_U && !pte->u);
  do { if (dflag_translate == dflag_mem && 0) do { printf("\33[1;34m[%s:%d,%s] " "ok: %i, mode == U: %i, pte->u: %i, ppn: %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 46, __func__, ok, mode == MODE_U, pte->u, (uint64_t)pte->ppn << 12); ; } while (0); if (dflag_translate == dflag_translate && 0) do { printf("\33[1;34m[%s:%d,%s] " "ok: %i, mode == U: %i, pte->u: %i, ppn: %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 46, __func__, ok, mode == MODE_U, pte->u, (uint64_t)pte->ppn << 12); ; } while (0); if (dflag_translate == dflag_trace_inst && 0) do { printf("\33[1;34m[%s:%d,%s] " "ok: %i, mode == U: %i, pte->u: %i, ppn: %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 46, __func__, ok, mode == MODE_U, pte->u, (uint64_t)pte->ppn << 12); ; } while (0); if (dflag_translate == dflag_trace_inst_dasm && 0) do { printf("\33[1;34m[%s:%d,%s] " "ok: %i, mode == U: %i, pte->u: %i, ppn: %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 46, __func__, ok, mode == MODE_U, pte->u, (uint64_t)pte->ppn << 12); ; } while (0); if (dflag_translate == dflag_trace_bb && 0) do { printf("\33[1;34m[%s:%d,%s] " "ok: %i, mode == U: %i, pte->u: %i, ppn: %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 46, __func__, ok, mode == MODE_U, pte->u, (uint64_t)pte->ppn << 12); ; } while (0); if (dflag_translate == dflag_betapoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "ok: %i, mode == U: %i, pte->u: %i, ppn: %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 46, __func__, ok, mode == MODE_U, pte->u, (uint64_t)pte->ppn << 12); ; } while (0); if (dflag_translate == dflag_exit && 0) do { printf("\33[1;34m[%s:%d,%s] " "ok: %i, mode == U: %i, pte->u: %i, ppn: %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 46, __func__, ok, mode == MODE_U, pte->u, (uint64_t)pte->ppn << 12); ; } while (0); if (dflag_translate == dflag_memprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "ok: %i, mode == U: %i, pte->u: %i, ppn: %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 46, __func__, ok, mode == MODE_U, pte->u, (uint64_t)pte->ppn << 12); ; } while (0); if (dflag_translate == dflag_ctrlprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "ok: %i, mode == U: %i, pte->u: %i, ppn: %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 46, __func__, ok, mode == MODE_U, pte->u, (uint64_t)pte->ppn << 12); ; } while (0); if (dflag_translate == dflag_simpoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "ok: %i, mode == U: %i, pte->u: %i, ppn: %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 46, __func__, ok, mode == MODE_U, pte->u, (uint64_t)pte->ppn << 12); ; } while (0); } while (0);
  ok = ok && !(pte->u && ((mode == MODE_S) && (!mstatus->sum || ifetch)));
  if (ifetch) {
    do { if (dflag_translate == dflag_mem && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for instr reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 49, __func__); ; } while (0); if (dflag_translate == dflag_translate && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for instr reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 49, __func__); ; } while (0); if (dflag_translate == dflag_trace_inst && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for instr reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 49, __func__); ; } while (0); if (dflag_translate == dflag_trace_inst_dasm && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for instr reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 49, __func__); ; } while (0); if (dflag_translate == dflag_trace_bb && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for instr reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 49, __func__); ; } while (0); if (dflag_translate == dflag_betapoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for instr reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 49, __func__); ; } while (0); if (dflag_translate == dflag_exit && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for instr reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 49, __func__); ; } while (0); if (dflag_translate == dflag_memprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for instr reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 49, __func__); ; } while (0); if (dflag_translate == dflag_ctrlprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for instr reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 49, __func__); ; } while (0); if (dflag_translate == dflag_simpoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for instr reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 49, __func__); ; } while (0); } while (0);
...
# 56 "src/isa/riscv64/system/mmu.c" 3 4
   _Bool 
# 56 "src/isa/riscv64/system/mmu.c"
        update_ad = 
# 56 "src/isa/riscv64/system/mmu.c" 3 4
                    0
# 56 "src/isa/riscv64/system/mmu.c"
...
# 59 "src/isa/riscv64/system/mmu.c" 3 4
     ((void) sizeof ((
# 59 "src/isa/riscv64/system/mmu.c"
     !cpu.amo
# 59 "src/isa/riscv64/system/mmu.c" 3 4
     ) ? 1 : 0), __extension__ ({ if (
# 59 "src/isa/riscv64/system/mmu.c"
...
# 59 "src/isa/riscv64/system/mmu.c"
     "!cpu.amo"
# 59 "src/isa/riscv64/system/mmu.c" 3 4
     , "src/isa/riscv64/system/mmu.c", 59, __extension__ __PRETTY_FUNCTION__); }))
# 59 "src/isa/riscv64/system/mmu.c"
                     ;
      (*((intr_deleg_S(EX_IPF)) ? (word_t *)stval : (word_t *)mtval)) = vaddr;
      longjmp_exception(EX_IPF);
      return 
# 62 "src/isa/riscv64/system/mmu.c" 3 4
            0
# 62 "src/isa/riscv64/system/mmu.c"
                 ;
    }
  } else if (type == MEM_TYPE_READ) {
    do { if (dflag_translate == dflag_mem && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 65, __func__); ; } while (0); if (dflag_translate == dflag_translate && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 65, __func__); ; } while (0); if (dflag_translate == dflag_trace_inst && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 65, __func__); ; } while (0); if (dflag_translate == dflag_trace_inst_dasm && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 65, __func__); ; } while (0); if (dflag_translate == dflag_trace_bb && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 65, __func__); ; } while (0); if (dflag_translate == dflag_betapoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 65, __func__); ; } while (0); if (dflag_translate == dflag_exit && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 65, __func__); ; } while (0); if (dflag_translate == dflag_memprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 65, __func__); ; } while (0); if (dflag_translate == dflag_ctrlprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 65, __func__); ; } while (0); if (dflag_translate == dflag_simpoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory reading" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 65, __func__); ; } while (0); } while (0);
    
# 66 "src/isa/riscv64/system/mmu.c" 3 4
   _Bool 
# 66 "src/isa/riscv64/system/mmu.c"
        can_load = pte->r || (mstatus->mxr && pte->x);
...
# 72 "src/isa/riscv64/system/mmu.c" 3 4
   _Bool 
# 72 "src/isa/riscv64/system/mmu.c"
        update_ad = 
# 72 "src/isa/riscv64/system/mmu.c" 3 4
                    0
# 72 "src/isa/riscv64/system/mmu.c"
    ...
    if (!(ok && can_load) || update_ad) {
      if (cpu.amo) do { if (dflag_translate == dflag_mem && 0) do { printf("\33[1;34m[%s:%d,%s] " "redirect to AMO page fault exception at pc = " "0x%016lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 75, __func__, cpu.pc); ; } while (0); if (dflag_translate == dflag_translate && 0) do { printf("\33[1;34m[%s:%d,%s] " "redirect to AMO page fault exception at pc = " "0x%016lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 75, __func__, cpu.pc); ; } while (0); if (dflag_translate == dflag_trace_inst && 0) do { printf("\33[1;34m[%s:%d,%s] " "redirect to AMO page fault exception at pc = " "0x%016lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 75, __func__, cpu.pc); ; } while (0); if (dflag_translate == dflag_trace_inst_dasm && 0) do { printf("\33[1;34m[%s:%d,%s] " "redirect to AMO page fault exception at pc = " "0x%016lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 75, __func__, cpu.pc); ; } while (0); if (dflag_translate == dflag_trace_bb && 0) do { printf("\33[1;34m[%s:%d,%s] " "redirect to AMO page fault exception at pc = " "0x%016lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 75, __func__, cpu.pc); ; } while (0); if (dflag_translate == dflag_betapoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "redirect to AMO page fault exception at pc = " "0x%016lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 75, __func__, cpu.pc); ; } while (0); if (dflag_translate == dflag_exit && 0) do { printf("\33[1;34m[%s:%d,%s] " "redirect to AMO page fault exception at pc = " "0x%016lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 75, __func__, cpu.pc); ; } while (0); if (dflag_translate == dflag_memprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "redirect to AMO page fault exception at pc = " "0x%016lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 75, __func__, cpu.pc); ; } while (0); if (dflag_translate == dflag_ctrlprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "redirect to AMO page fault exception at pc = " "0x%016lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 75, __func__, cpu.pc); ; } while (0); if (dflag_translate == dflag_simpoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "redirect to AMO page fault exception at pc = " "0x%016lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 75, __func__, cpu.pc); ; } while (0); } while (0);
      int ex = (cpu.amo ? EX_SPF : EX_LPF);
      (*((intr_deleg_S(ex)) ? (word_t *)stval : (word_t *)mtval)) = vaddr;
      cpu.amo = 
# 78 "src/isa/riscv64/system/mmu.c" 3 4
               0
# 78 "src/isa/riscv64/system/mmu.c"
                    ;
      do { if (dflag_translate == dflag_mem && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory read translation exception!" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 79, __func__); ; } while (0); if (dflag_translate == dflag_translate && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory read translation exception!" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 79, __func__); ; } while (0); if (dflag_translate == dflag_trace_inst && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory read translation exception!" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 79, __func__); ; } while (0); if (dflag_translate == dflag_trace_inst_dasm && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory read translation exception!" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 79, __func__); ; } while (0); if (dflag_translate == dflag_trace_bb && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory read translation exception!" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 79, __func__); ; } while (0); if (dflag_translate == dflag_betapoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory read translation exception!" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 79, __func__); ; } while (0); if (dflag_translate == dflag_exit && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory read translation exception!" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 79, __func__); ; } while (0); if (dflag_translate == dflag_memprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory read translation exception!" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 79, __func__); ; } while (0); if (dflag_translate == dflag_ctrlprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory read translation exception!" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 79, __func__); ; } while (0); if (dflag_translate == dflag_simpoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory read translation exception!" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 79, __func__); ; } while (0); } while (0);
      longjmp_exception(ex);
      return 
# 81 "src/isa/riscv64/system/mmu.c" 3 4
            0
# 81 "src/isa/riscv64/system/mmu.c"
                 ;
    }
  } else {
...
# 88 "src/isa/riscv64/system/mmu.c" 3 4
   _Bool 
# 88 "src/isa/riscv64/system/mmu.c"
        update_ad = 
# 88 "src/isa/riscv64/system/mmu.c" 3 4
                    0
# 88 "src/isa/riscv64/system/mmu.c"
    ...
    do { if (dflag_translate == dflag_mem && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory writing" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 90, __func__); ; } while (0); if (dflag_translate == dflag_translate && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory writing" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 90, __func__); ; } while (0); if (dflag_translate == dflag_trace_inst && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory writing" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 90, __func__); ; } while (0); if (dflag_translate == dflag_trace_inst_dasm && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory writing" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 90, __func__); ; } while (0); if (dflag_translate == dflag_trace_bb && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory writing" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 90, __func__); ; } while (0); if (dflag_translate == dflag_betapoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory writing" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 90, __func__); ; } while (0); if (dflag_translate == dflag_exit && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory writing" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 90, __func__); ; } while (0); if (dflag_translate == dflag_memprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory writing" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 90, __func__); ; } while (0); if (dflag_translate == dflag_ctrlprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory writing" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 90, __func__); ; } while (0); if (dflag_translate == dflag_simpoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Translate for memory writing" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 90, __func__); ; } while (0); } while (0);
    if (!(ok && pte->w) || update_ad) {
      (*((intr_deleg_S(EX_SPF)) ? (word_t *)stval : (word_t *)mtval)) = vaddr;
      cpu.amo = 
# 93 "src/isa/riscv64/system/mmu.c" 3 4
               0
# 93 "src/isa/riscv64/system/mmu.c"
                    ;
      longjmp_exception(EX_SPF);
      return 
# 95 "src/isa/riscv64/system/mmu.c" 3 4
            0
# 95 "src/isa/riscv64/system/mmu.c"
                 ;
    }
  }
  return 
# 98 "src/isa/riscv64/system/mmu.c" 3 4
        1
# 98 "src/isa/riscv64/system/mmu.c"
            ;
}

static paddr_t ptw(vaddr_t vaddr, int type) {
  do { if (dflag_translate == dflag_mem && 0) do { printf("\33[1;34m[%s:%d,%s] " "Page walking for 0x%lx\n" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 102, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_translate && 0) do { printf("\33[1;34m[%s:%d,%s] " "Page walking for 0x%lx\n" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 102, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_trace_inst && 0) do { printf("\33[1;34m[%s:%d,%s] " "Page walking for 0x%lx\n" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 102, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_trace_inst_dasm && 0) do { printf("\33[1;34m[%s:%d,%s] " "Page walking for 0x%lx\n" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 102, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_trace_bb && 0) do { printf("\33[1;34m[%s:%d,%s] " "Page walking for 0x%lx\n" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 102, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_betapoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Page walking for 0x%lx\n" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 102, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_exit && 0) do { printf("\33[1;34m[%s:%d,%s] " "Page walking for 0x%lx\n" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 102, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_memprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Page walking for 0x%lx\n" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 102, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_ctrlprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Page walking for 0x%lx\n" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 102, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_simpoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Page walking for 0x%lx\n" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 102, __func__, vaddr); ; } while (0); } while (0);
  word_t pg_base = (satp->ppn << 12);
  word_t p_pte;
  PTE pte;
  int level;
  ...
  for (level = 3 - 1; level >= 0;) {
    p_pte = pg_base + VPNi(vaddr, level) * 8;



    pte.val = paddr_read(p_pte, 8,
  ...
  if (!check_permission(&pte, 
# 134 "src/isa/riscv64/system/mmu.c" 3 4
                             1
# 134 "src/isa/riscv64/system/mmu.c"
                                 , vaddr, type)) return MEM_RET_FAIL;
  ...
  if (level > 0) {
    ...
    pg_base = (pg_base & ~pg_mask) | (vaddr & pg_mask & ~((1ull << 12) - 1));
  }



  
# 148 "src/isa/riscv64/system/mmu.c" 3 4
 _Bool 
# 148 "src/isa/riscv64/system/mmu.c"
      is_write = (type == MEM_TYPE_WRITE);
  if (!pte.a || (!pte.d && is_write)) {
    pte.a = 
# 150 "src/isa/riscv64/system/mmu.c" 3 4
           1
# 150 "src/isa/riscv64/system/mmu.c"
               ;
    pte.d |= is_write;
    paddr_write(p_pte, 8, pte.val, cpu.mode, vaddr);
  }
...
bad:
  do { if (dflag_translate == dflag_mem && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory translation bad" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 159, __func__); ; } while (0); if (dflag_translate == dflag_translate && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory translation bad" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 159, __func__); ; } while (0); if (dflag_translate == dflag_trace_inst && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory translation bad" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 159, __func__); ; } while (0); if (dflag_translate == dflag_trace_inst_dasm && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory translation bad" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 159, __func__); ; } while (0); if (dflag_translate == dflag_trace_bb && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory translation bad" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 159, __func__); ; } while (0); if (dflag_translate == dflag_betapoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory translation bad" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 159, __func__); ; } while (0); if (dflag_translate == dflag_exit && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory translation bad" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 159, __func__); ; } while (0); if (dflag_translate == dflag_memprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory translation bad" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 159, __func__); ; } while (0); if (dflag_translate == dflag_ctrlprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory translation bad" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 159, __func__); ; } while (0); if (dflag_translate == dflag_simpoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "Memory translation bad" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 159, __func__); ; } while (0); } while (0);
  check_permission(&pte, 
# 160 "src/isa/riscv64/system/mmu.c" 3 4
                        0
# 160 "src/isa/riscv64/system/mmu.c"
                             , vaddr, type);
  return MEM_RET_FAIL;
}
...
static inline int update_mmu_state_internal(
# 171 "src/isa/riscv64/system/mmu.c" 3 4
                                           _Bool 
# 171 "src/isa/riscv64/system/mmu.c"
                                                ifetch) {
  uint32_t mode = (mstatus->mprv && (!ifetch) ? mstatus->mpp : cpu.mode);
  if (mode < MODE_M) {
    
# 174 "src/isa/riscv64/system/mmu.c" 3 4
   ((void) sizeof ((
# 174 "src/isa/riscv64/system/mmu.c"
   satp->mode == 0 || satp->mode == 8
# 174 "src/isa/riscv64/system/mmu.c" 3 4
   ) ? 1 : 0), __extension__ ({ if (
# 174 "src/isa/riscv64/system/mmu.c"
   satp->mode == 0 || satp->mode == 8
...
# 174 "src/isa/riscv64/system/mmu.c"
   "satp->mode == 0 || satp->mode == 8"
# 174 "src/isa/riscv64/system/mmu.c" 3 4
   , "src/isa/riscv64/system/mmu.c", 174, __extension__ __PRETTY_FUNCTION__); }))
# 174 "src/isa/riscv64/system/mmu.c"
                                             ;
...
int update_mmu_state() {
  ifetch_mmu_state = update_mmu_state_internal(
# 181 "src/isa/riscv64/system/mmu.c" 3 4
                                              1
# 181 "src/isa/riscv64/system/mmu.c"
                                                  );
  int data_mmu_state_old = data_mmu_state;
  data_mmu_state = update_mmu_state_internal(
# 183 "src/isa/riscv64/system/mmu.c" 3 4
                                            0
# 183 "src/isa/riscv64/system/mmu.c"
                                                 );
  return (data_mmu_state ^ data_mmu_state_old) ? 
# 184 "src/isa/riscv64/system/mmu.c" 3 4
                                                1 
# 184 "src/isa/riscv64/system/mmu.c"
                                                     : 
# 184 "src/isa/riscv64/system/mmu.c" 3 4
                                                       0
# 184 "src/isa/riscv64/system/mmu.c"
...
int isa_mmu_check(vaddr_t vaddr, int len, int type) {
  do { if (dflag_translate == dflag_mem && 0) do { printf("\33[1;34m[%s:%d,%s] " "MMU checking addr %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 188, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_translate && 0) do { printf("\33[1;34m[%s:%d,%s] " "MMU checking addr %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 188, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_trace_inst && 0) do { printf("\33[1;34m[%s:%d,%s] " "MMU checking addr %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 188, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_trace_inst_dasm && 0) do { printf("\33[1;34m[%s:%d,%s] " "MMU checking addr %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 188, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_trace_bb && 0) do { printf("\33[1;34m[%s:%d,%s] " "MMU checking addr %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 188, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_betapoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "MMU checking addr %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 188, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_exit && 0) do { printf("\33[1;34m[%s:%d,%s] " "MMU checking addr %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 188, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_memprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "MMU checking addr %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 188, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_ctrlprof && 0) do { printf("\33[1;34m[%s:%d,%s] " "MMU checking addr %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 188, __func__, vaddr); ; } while (0); if (dflag_translate == dflag_simpoint && 0) do { printf("\33[1;34m[%s:%d,%s] " "MMU checking addr %lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 188, __func__, vaddr); ; } while (0); } while (0);
  
# 189 "src/isa/riscv64/system/mmu.c" 3 4
 _Bool 
# 189 "src/isa/riscv64/system/mmu.c"
      is_ifetch = type == MEM_TYPE_IFETCH;



  
# 193 "src/isa/riscv64/system/mmu.c" 3 4
 _Bool 
# 193 "src/isa/riscv64/system/mmu.c"
      vm_enable = (mstatus->mprv && (!is_ifetch) ? mstatus->mpp : cpu.mode) < MODE_M && satp->mode == 8;
  word_t va_mask = ((((word_t)1) << (63 - 38 + 1)) - 1);
  word_t va_msbs = vaddr >> 38;
  
# 196 "src/isa/riscv64/system/mmu.c" 3 4
 _Bool 
# 196 "src/isa/riscv64/system/mmu.c"
      va_msbs_ok = (va_msbs == va_mask) || va_msbs == 0 || !vm_enable;
  ...
  if (1 && __builtin_expect((vaddr & (len - 1)) != 0, 0)) {
    do { printf("\33[1;34m[%s:%d,%s] " "addr misaligned happened: vaddr:%lx len:%d type:%d pc:%lx" "\33[0m\n", "src/isa/riscv64/system/mmu.c", 216, __func__, vaddr, len, type, cpu.pc); ; } while (0);
...
# 236 "src/isa/riscv64/system/mmu.c"
paddr_t isa_mmu_translate(vaddr_t vaddr, int len, int type) {
...
# 257 "src/isa/riscv64/system/mmu.c" 3 4
 _Bool 
# 257 "src/isa/riscv64/system/mmu.c"
      ifetch = (type == MEM_TYPE_IFETCH);
  ...
  if(cpu.guided_exec && cpu.execution_guide.force_raise_exception){
    if(ifetch && cpu.execution_guide.exception_num == EX_IPF){
      ...
      (*((intr_deleg_S(EX_LPF)) ? (word_t *)stval : (word_t *)mtval)) = vaddr;
...
# 313 "src/isa/riscv64/system/mmu.c" 3 4
_Bool 
# 313 "src/isa/riscv64/system/mmu.c"
    isa_pmp_check_permission(paddr_t addr, int len, int type, int out_mode) {
  
# 314 "src/isa/riscv64/system/mmu.c" 3 4
 _Bool 
# 314 "src/isa/riscv64/system/mmu.c"
      ifetch = (type == MEM_TYPE_IFETCH);
  __attribute__((unused)) uint32_t mode;
  mode = (out_mode == MODE_M) ? (mstatus->mprv && !ifetch ? mstatus->mpp : cpu.mode) : out_mode;
# 413 "src/isa/riscv64/system/mmu.c"
  return 
# 413 "src/isa/riscv64/system/mmu.c" 3 4
        1
# 413 "src/isa/riscv64/system/mmu.c"
            ;

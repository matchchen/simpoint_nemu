...
static Decode tcache_pool[8192] = {};
static int tc_idx = 0;
static Decode tcache_bb_pool[(8192 / 4 + 2)] = {};
static Decode *tcache_bb_freelist = 
# 19 "src/cpu/tcache.c" 3 4
                                   ((void *)0)
# 19 "src/cpu/tcache.c"
                                       ;
static bb_t bb_pool[1024] = {};
static int bb_idx = 0;
static bb_t bb_list [1024] = {};
static const void *g_exec_nemu_decode;

static inline Decode* tcache_entry_init(Decode *s, vaddr_t pc) {
  s->tnext = s->ntnext = 
# 26 "src/cpu/tcache.c" 3 4
                        ((void *)0)
# 26 "src/cpu/tcache.c"
                            ;
  s->type = 0;
...
static inline Decode* tcache_new(vaddr_t pc) {
  if (tc_idx == 8192) return 
# 43 "src/cpu/tcache.c" 3 4
                                          ((void *)0)
# 43 "src/cpu/tcache.c"
                                              ;
  
# 44 "src/cpu/tcache.c" 3 4
 ((void) sizeof ((
# 44 "src/cpu/tcache.c"
 tc_idx < 8192
# 44 "src/cpu/tcache.c" 3 4
...
# 44 "src/cpu/tcache.c"
 "tc_idx < CONFIG_TCACHE_SIZE"
# 44 "src/cpu/tcache.c" 3 4
 , "src/cpu/tcache.c", 44, __extension__ __PRETTY_FUNCTION__); }))
# 44 "src/cpu/tcache.c"
                                    ;
...
# 61 "src/cpu/tcache.c"
 "s != NULL"
# 61 "src/cpu/tcache.c" 3 4
 , "src/cpu/tcache.c", 61, __extension__ __PRETTY_FUNCTION__); }))
# 61 "src/cpu/tcache.c"
                  ;
  do { int idx = s - tcache_bb_pool; do { if (!(idx >= 0 && idx < (8192 / 4 + 2))) { fflush(
# 62 "src/cpu/tcache.c" 3 4
 stdout
# 62 "src/cpu/tcache.c"
 ); fprintf(
# 62 "src/cpu/tcache.c" 3 4
...
# 62 "src/cpu/tcache.c"
 "idx >= 0 && idx < (8192 / 4 + 2)"
# 62 "src/cpu/tcache.c" 3 4
 , "src/cpu/tcache.c", 62, __extension__ __PRETTY_FUNCTION__); }))
# 62 "src/cpu/tcache.c"
 ; } } while (0); } while (0);
  do { int idx = tcache_bb_freelist->tnext - tcache_bb_pool; do { if (!(idx >= 0 && idx < (8192 / 4 + 2))) { fflush(
# 63 "src/cpu/tcache.c" 3 4
 stdout
# 63 "src/cpu/tcache.c"
 ); fprintf(
# 63 "src/cpu/tcache.c" 3 4
...
# 63 "src/cpu/tcache.c" 3 4
 stderr
# 63 "src/cpu/tcache.c"
 , "idx = %d, s = %p", idx, tcache_bb_freelist->tnext); fprintf(
# 63 "src/cpu/tcache.c" 3 4
 stderr
# 63 "src/cpu/tcache.c"
 , "\33[0m\n"); extern void isa_reg_display(); extern void monitor_statistic(); isa_reg_display(); monitor_statistic(); 
...
# 63 "src/cpu/tcache.c"
 "idx >= 0 && idx < (8192 / 4 + 2)"
# 63 "src/cpu/tcache.c" 3 4
 , "src/cpu/tcache.c", 63, __extension__ __PRETTY_FUNCTION__); }))
# 63 "src/cpu/tcache.c"
 ; } } while (0); } while (0);
  tcache_bb_freelist = tcache_bb_freelist->tnext;
  do { int idx = tcache_bb_freelist - tcache_bb_pool; do { if (!(idx >= 0 && idx < (8192 / 4 + 2))) { fflush(
# 65 "src/cpu/tcache.c" 3 4
 stdout
# 65 "src/cpu/tcache.c"
 ); fprintf(
# 65 "src/cpu/tcache.c" 3 4
...
# 65 "src/cpu/tcache.c" 3 4
 stderr
# 65 "src/cpu/tcache.c"
 , "idx = %d, s = %p", idx, tcache_bb_freelist); fprintf(
# 65 "src/cpu/tcache.c" 3 4
 stderr
# 65 "src/cpu/tcache.c"
 , "\33[0m\n"); extern void isa_reg_display(); extern void monitor_statistic(); isa_reg_display(); monitor_statistic(); 
...
# 65 "src/cpu/tcache.c"
 "idx >= 0 && idx < (8192 / 4 + 2)"
# 65 "src/cpu/tcache.c" 3 4
 , "src/cpu/tcache.c", 65, __extension__ __PRETTY_FUNCTION__); }))
# 65 "src/cpu/tcache.c"
 ; } } while (0); } while (0);
  do { int idx = tcache_bb_freelist->tnext - tcache_bb_pool; do { if (!(idx >= 0 && idx < (8192 / 4 + 2))) { fflush(
# 66 "src/cpu/tcache.c" 3 4
 stdout
# 66 "src/cpu/tcache.c"
 ); fprintf(
# 66 "src/cpu/tcache.c" 3 4
...
# 66 "src/cpu/tcache.c"
 "idx >= 0 && idx < (8192 / 4 + 2)"
# 66 "src/cpu/tcache.c" 3 4
 , "src/cpu/tcache.c", 66, __extension__ __PRETTY_FUNCTION__); }))
# 66 "src/cpu/tcache.c"
 ; } } while (0); } while (0);
  return tcache_entry_init(s, pc);
}
...
# 71 "src/cpu/tcache.c"
 "idx >= 0 && idx < (8192 / 4 + 2)"
# 71 "src/cpu/tcache.c" 3 4
 , "src/cpu/tcache.c", 71, __extension__ __PRETTY_FUNCTION__); }))
# 71 "src/cpu/tcache.c"
 ; } } while (0); } while (0);
  do { int idx = tcache_bb_freelist - tcache_bb_pool; do { if (!(idx >= 0 && idx < (8192 / 4 + 2))) { fflush(
# 72 "src/cpu/tcache.c" 3 4
 stdout
# 72 "src/cpu/tcache.c"
 ); fprintf(
# 72 "src/cpu/tcache.c" 3 4
...
# 72 "src/cpu/tcache.c"
 "idx >= 0 && idx < (8192 / 4 + 2)"
# 72 "src/cpu/tcache.c" 3 4
 , "src/cpu/tcache.c", 72, __extension__ __PRETTY_FUNCTION__); }))
# 72 "src/cpu/tcache.c"
 ; } } while (0); } while (0);
  s->tnext = tcache_bb_freelist;
  tcache_bb_freelist = s;
  do { int idx = tcache_bb_freelist->tnext - tcache_bb_pool; do { if (!(idx >= 0 && idx < (8192 / 4 + 2))) { fflush(
# 75 "src/cpu/tcache.c" 3 4
 stdout
# 75 "src/cpu/tcache.c"
 ); fprintf(
# 75 "src/cpu/tcache.c" 3 4
...
# 75 "src/cpu/tcache.c"
 "idx >= 0 && idx < (8192 / 4 + 2)"
# 75 "src/cpu/tcache.c" 3 4
 , "src/cpu/tcache.c", 75, __extension__ __PRETTY_FUNCTION__); }))
# 75 "src/cpu/tcache.c"
 ; } } while (0); } while (0);
}
...
static inline bb_t* bb_new(Decode *s, vaddr_t pc, bb_t *next) {
  if (bb_idx == 1024) return 
# 80 "src/cpu/tcache.c" 3 4
                                           ((void *)0)
# 80 "src/cpu/tcache.c"
                                               ;
  
# 81 "src/cpu/tcache.c" 3 4
 ((void) sizeof ((
# 81 "src/cpu/tcache.c"
 bb_idx < 1024
# 81 "src/cpu/tcache.c" 3 4
...
# 81 "src/cpu/tcache.c"
 "bb_idx < CONFIG_BB_POOL_SIZE"
# 81 "src/cpu/tcache.c" 3 4
 , "src/cpu/tcache.c", 81, __extension__ __PRETTY_FUNCTION__); }))
# 81 "src/cpu/tcache.c"
                                     ;
...
static struct bb_t* bb_insert(vaddr_t pc, Decode *fill) {
  ...
  if (bb == 
# 106 "src/cpu/tcache.c" 3 4
           ((void *)0)
# 106 "src/cpu/tcache.c"
               ) return 
# 106 "src/cpu/tcache.c" 3 4
                        ((void *)0)
# 106 "src/cpu/tcache.c"
...
static bb_t* bb_find(vaddr_t pc) {
  bb_t *bb = bb_hash(pc);
  if (__builtin_expect(bb->pc == pc, 1)) return bb;
  bb_t *head = bb;
  do {
    bb = bb->next;
    if (bb == (void *)-1ul) return 
# 119 "src/cpu/tcache.c" 3 4
                                  ((void *)0)
# 119 "src/cpu/tcache.c"
                                      ;
    if (bb->pc == pc) {
...
static void tcache_bb_fetch(Decode *_this, int is_taken, vaddr_t jpc) {
  bb_t* bb = bb_find(jpc);
  if (bb != 
# 131 "src/cpu/tcache.c" 3 4
           ((void *)0)
# 131 "src/cpu/tcache.c"
               ) {
...
void tcache_flush() {
  ...
  int i;
  for (i = 0; i < (8192 / 4 + 2) - 1; i ++) {
    tcache_bb_pool[i].list_next = &tcache_bb_pool[i + 1];
  }
  tcache_bb_pool[(8192 / 4 + 2) - 1].list_next = 
# 151 "src/cpu/tcache.c" 3 4
                                                ((void *)0)
# 151 "src/cpu/tcache.c"
                                                    ;
...
static Decode *bb_now = 
# 157 "src/cpu/tcache.c" 3 4
                       ((void *)0)
# 157 "src/cpu/tcache.c"
                           , *bb_now_record = 
# 157 "src/cpu/tcache.c" 3 4
                                              ((void *)0)
# 157 "src/cpu/tcache.c"
...
Decode* tcache_jr_fetch(Decode *s, vaddr_t jpc) {
  s->ntnext = s->tnext;
  tcache_bb_fetch(s, 
# 162 "src/cpu/tcache.c" 3 4
                    1
# 162 "src/cpu/tcache.c"
                        , jpc);
  return s->tnext;
}
...
Decode* tcache_decode(Decode *s) {
  ...
  if (tcache_state == TCACHE_RUNNING) {
    ...
    if (bb != 
# 181 "src/cpu/tcache.c" 3 4
             ((void *)0)
# 181 "src/cpu/tcache.c"
                 ) {
    ...
    idx_in_bb = 1;
    if (s == 
# 189 "src/cpu/tcache.c" 3 4
            ((void *)0)
# 189 "src/cpu/tcache.c"
                ) { goto full; }
  ...
  fetch_decode(s, thispc);

  if (s->type == INSTR_TYPE_N) {
    Decode *next = tcache_new(s->snpc);
    if (next == 
# 202 "src/cpu/tcache.c" 3 4
               ((void *)0)
# 202 "src/cpu/tcache.c"
                   ) { goto full; }
    
# 203 "src/cpu/tcache.c" 3 4
   ((void) sizeof ((
# 203 "src/cpu/tcache.c"
   next == s + 1
# 203 "src/cpu/tcache.c" 3 4
   ) ? 1 : 0), __extension__ ({ if (
# 203 "src/cpu/tcache.c"
...
# 203 "src/cpu/tcache.c"
   "next == s + 1"
# 203 "src/cpu/tcache.c" 3 4
   , "src/cpu/tcache.c", 203, __extension__ __PRETTY_FUNCTION__); }))
# 203 "src/cpu/tcache.c"
                        ;
  } else {
    ...
    if (ret == 
# 207 "src/cpu/tcache.c" 3 4
              ((void *)0)
# 207 "src/cpu/tcache.c"
                  ) { goto full; }
    tcache_patch_and_free(bb_now_record, bb_now);
    bb_now = bb_now_record = 
# 209 "src/cpu/tcache.c" 3 4
                            ((void *)0)
# 209 "src/cpu/tcache.c"
                                ;

    switch (s->type) {
      case INSTR_TYPE_J: tcache_bb_fetch(s, 
# 212 "src/cpu/tcache.c" 3 4
                                           1
# 212 "src/cpu/tcache.c"
                                               , s->jnpc); break;
      case INSTR_TYPE_B:
        tcache_bb_fetch(s, 
# 214 "src/cpu/tcache.c" 3 4
                          1
# 214 "src/cpu/tcache.c"
                              , s->jnpc);
        tcache_bb_fetch(s, 
# 215 "src/cpu/tcache.c" 3 4
                          0
# 215 "src/cpu/tcache.c"
                               , s->snpc + 0);
        break;
      case INSTR_TYPE_I: s->tnext = s->ntnext = s; break;
      default: 
# 218 "src/cpu/tcache.c" 3 4
              ((void) sizeof ((
# 218 "src/cpu/tcache.c"
              0
# 218 "src/cpu/tcache.c" 3 4
              ) ? 1 : 0), __extension__ ({ if (
# 218 "src/cpu/tcache.c"
...
# 218 "src/cpu/tcache.c"
              "0"
# 218 "src/cpu/tcache.c" 3 4
              , "src/cpu/tcache.c", 218, __extension__ __PRETTY_FUNCTION__); }))
# 218 "src/cpu/tcache.c"
                       ;
    }
...
full:
  tcache_flush();
  s = tcache_bb_new(thispc);
  s->idx_in_bb = idx_in_bb;
  save_globals(s);
  bb_now = bb_now_record = 
# 231 "src/cpu/tcache.c" 3 4
                          ((void *)0)
# 231 "src/cpu/tcache.c"
                              ;
  tcache_state = TCACHE_RUNNING;
...
void tcache_handle_exception(vaddr_t jpc) {
  tcache_bb_fetch(&ex, 
# 239 "src/cpu/tcache.c" 3 4
                      1
# 239 "src/cpu/tcache.c"
                          , jpc);

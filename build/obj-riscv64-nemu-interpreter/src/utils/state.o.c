...
int is_exit_status_bad() {
  ...
  if (!good) {
    do { printf("\33[1;34m[%s:%d,%s] " "NEMU exit with bad state: %i, halt ret: %i" "\33[0m\n", "src/utils/state.c", 9, __func__, nemu_state.state, nemu_state.halt_ret); ; } while (0);
  } else {
    do { printf("\33[1;34m[%s:%d,%s] " "NEMU exit with good state: %i, halt ret: %i" "\33[0m\n", "src/utils/state.c", 11, __func__, nemu_state.state, nemu_state.halt_ret); ; } while (0);
  }
  return !good;
}

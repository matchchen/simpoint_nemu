...
# 5 "/home/user/ecpt/work/simpoint_nemu/include/cpu/difftest.h" 2
...
# 6 "/home/user/ecpt/work/simpoint_nemu/lib-include/difftest.h"
...
# 2 "src/device/sdcard.c" 2
# 1 "src/device/mmc.h" 1
# 3 "src/device/sdcard.c" 2
# 19 "src/device/sdcard.c"
enum {
...
static FILE *fp = 
# 28 "src/device/sdcard.c" 3 4
                 ((void *)0)
# 28 "src/device/sdcard.c"
                     ;
static uint32_t *base = 
# 29 "src/device/sdcard.c" 3 4
                       ((void *)0)
# 29 "src/device/sdcard.c"
                           ;
static uint32_t blkcnt = 0;
static long blk_addr = 0;
static uint32_t addr = 0;
static 
# 33 "src/device/sdcard.c" 3 4
      _Bool 
# 33 "src/device/sdcard.c"
           write_cmd = 0;
static 
# 34 "src/device/sdcard.c" 3 4
      _Bool 
# 34 "src/device/sdcard.c"
           read_ext_csd = 
# 34 "src/device/sdcard.c" 3 4
                          0
# 34 "src/device/sdcard.c"
...
static inline void prepare_rw(int is_write) {
  blk_addr = base[SDARG];
  addr = 0;
  if (fp) fseek(fp, blk_addr << 9, 
# 39 "src/device/sdcard.c" 3 4
                                  0
# 39 "src/device/sdcard.c"
                                          );
  write_cmd = is_write;
}
...
static void sdcard_handle_cmd(int cmd) {
  switch (cmd) {
    case 0: break;
    case 1: base[SDRSP0] = 0x80ff8000; break;
    case 2:
    ...
    case 9:
      ...
      base[SDRSP1] = 0x124b97e3 | (((((16ull * 1024 * 1024 * 1024) / (1 << 15)) / (1 << (7 + 2)) - 1) & 0x3) << 30);
      base[SDRSP2] = 0x0f508000 | ((((16ull * 1024 * 1024 * 1024) / (1 << 15)) / (1 << (7 + 2)) - 1) >> 2) | (15 << 16);
      base[SDRSP3] = 0x9026012a;
      break;
    case 8: read_ext_csd = 
# 61 "src/device/sdcard.c" 3 4
                                         1
# 61 "src/device/sdcard.c"
                                             ; addr = 0; break;
    case 5: break;
    case 55: break;
    case 3: break;
    case 7: break;
    case 23: blkcnt = base[SDARG] & 0xffff; break;
    case 18: prepare_rw(
# 67 "src/device/sdcard.c" 3 4
                                            0
# 67 "src/device/sdcard.c"
                                                 ); break;
    case 25: prepare_rw(
# 68 "src/device/sdcard.c" 3 4
                                             1
# 68 "src/device/sdcard.c"
                                                 ); break;
    case 13: base[SDRSP0] = base[SDRSP1] = base[SDRSP2] = base[SDRSP3] = 0; break;
    case 12: break;
    default:
      do { if (!(0)) { fflush(
# 72 "src/device/sdcard.c" 3 4
     stdout
# 72 "src/device/sdcard.c"
     ); fprintf(
# 72 "src/device/sdcard.c" 3 4
     stderr
# 72 "src/device/sdcard.c"
...
# 72 "src/device/sdcard.c" 3 4
     stderr
# 72 "src/device/sdcard.c"
     , "unhandled command = %d", cmd); fprintf(
# 72 "src/device/sdcard.c" 3 4
     stderr
# 72 "src/device/sdcard.c"
...
# 72 "src/device/sdcard.c"
     "0"
# 72 "src/device/sdcard.c" 3 4
     , "src/device/sdcard.c", 72, __extension__ __PRETTY_FUNCTION__); }))
# 72 "src/device/sdcard.c"
     ; } } while (0);
  }
}

static void sdcard_io_handler(uint32_t offset, int len, 
# 76 "src/device/sdcard.c" 3 4
                                                       _Bool 
# 76 "src/device/sdcard.c"
                                                            is_write) {
  int idx = offset / 4;
  switch (idx) {
    ...
    case SDDATA:
       if (read_ext_csd) {
         ...
         uint32_t data;
         switch (addr) {
           case 192: data = 2; break;
           case 212: data = (16ull * 1024 * 1024 * 1024) / 512; break;
           default: data = 0;
         }
         base[SDDATA] = data;
         if (addr == 512 - 4) read_ext_csd = 
# 96 "src/device/sdcard.c" 3 4
                                            0
# 96 "src/device/sdcard.c"
                                                 ;
       } else if (fp) {
       ...
       break;
    default:
      do { printf("\33[1;34m[%s:%d,%s] " "offset = 0x%x(idx = %d), is_write = %d, data = 0x%x" "\33[0m\n", "src/device/sdcard.c", 105, __func__, offset, idx, is_write, base[idx]); ; } while (0);
      do { if (!(0)) { fflush(
# 106 "src/device/sdcard.c" 3 4
     stdout
# 106 "src/device/sdcard.c"
     ); fprintf(
# 106 "src/device/sdcard.c" 3 4
     stderr
# 106 "src/device/sdcard.c"
...
# 106 "src/device/sdcard.c" 3 4
     stderr
# 106 "src/device/sdcard.c"
     , "unhandle offset = %d", offset); fprintf(
# 106 "src/device/sdcard.c" 3 4
     stderr
# 106 "src/device/sdcard.c"
     , "\33[0m\n"); extern void isa_reg_display(); extern void monitor_statistic(); isa_reg_display(); monitor_statistic(); 
...
# 106 "src/device/sdcard.c"
     "0"
# 106 "src/device/sdcard.c" 3 4
     , "src/device/sdcard.c", 106, __extension__ __PRETTY_FUNCTION__); }))
# 106 "src/device/sdcard.c"
     ; } } while (0);
  }
}
...
void init_sdcard() {
  ...
  add_mmio_map("sdhci", 0xa3000000, base, 0x80, sdcard_io_handler);



  do { if (!((((16ull * 1024 * 1024 * 1024) / (1 << 15)) / (1 << (7 + 2)) - 1) < (1 << 12))) { fflush(
# 116 "src/device/sdcard.c" 3 4
 stdout
# 116 "src/device/sdcard.c"
 ); fprintf(
# 116 "src/device/sdcard.c" 3 4
 stderr
# 116 "src/device/sdcard.c"
...
# 116 "src/device/sdcard.c" 3 4
 stderr
# 116 "src/device/sdcard.c"
 , "shoule be fit in 12 bits"); fprintf(
# 116 "src/device/sdcard.c" 3 4
 stderr
# 116 "src/device/sdcard.c"
 , "\33[0m\n"); extern void isa_reg_display(); extern void monitor_statistic(); isa_reg_display(); monitor_statistic(); 
# 116 "src/device/sdcard.c" 3 4
 ((void) sizeof ((
# 116 "src/device/sdcard.c"
 (((16ull * 1024 * 1024 * 1024) / (1 << 15)) / (1 << (7 + 2)) - 1) < (1 << 12)
# 116 "src/device/sdcard.c" 3 4
 ) ? 1 : 0), __extension__ ({ if (
# 116 "src/device/sdcard.c"
 (((16ull * 1024 * 1024 * 1024) / (1 << 15)) / (1 << (7 + 2)) - 1) < (1 << 12)
...
# 116 "src/device/sdcard.c"
 "(((16ull * 1024 * 1024 * 1024) / (1 << 15)) / (1 << (7 + 2)) - 1) < (1 << 12)"
# 116 "src/device/sdcard.c" 3 4
 , "src/device/sdcard.c", 116, __extension__ __PRETTY_FUNCTION__); }))
# 116 "src/device/sdcard.c"
 ; } } while (0);
  ...
  fp = fopen(img, "r+");
  if (fp == 
# 120 "src/device/sdcard.c" 3 4
           ((void *)0)
# 120 "src/device/sdcard.c"
               ) {
      do { printf("\33[1;34m[%s:%d,%s] " "Can not find sdcard image: %s" "\33[0m\n", "src/device/sdcard.c", 121, __func__, img); ; } while (0);
  } else {
      do { printf("\33[1;34m[%s:%d,%s] " "Using sdcard image: %s" "\33[0m\n", "src/device/sdcard.c", 123, __func__, img); ; } while (0);
  }
}
